...
export default function VideoConference() {
  ...
  useEffect(() => {
    if (isSmallScreen) {
      if(activeParticipant){setThumbnailCount(4)}
      else {setThumbnailCount(7)}
    } else if (isMobile) {
      if(activeParticipant){setThumbnailCount(4)}
      else {setThumbnailCount(7)}
    } else if (isTablet) {
          ...
          {isSidebarOpen && (
            ...
            >
              <SidebarPanel
                ...
                part={sidebarOpenedFromParticipants ? "participants" : undefined}
              />
            </motion.div>
          )}
        </AnimatePresence>
                  ...
                  >
                    {gridParticipants.map((participant) => (
                      <div
                        key={participant.id}
                        className={cn(
                          gridParticipants.length === 1 && !activeParticipant ? "w-full h-full" : "",
                          activeParticipant ? "flex-shrink-0 w-1/5" : "",
                          "relative transition-all rounded-[20px]",
                          participant.id === activeParticipant?.id ? "border-4 border-green-500" : "",
                          participants.length >= 4 ? "cursor-pointer" : "cursor-default"
                        )}
                        onClick={participants.length >= 4 ? () => setAsActive(participant) : undefined}
                      >
                        <ParticipantVideo
                          participant={participant}
                          isVideoOn={() => setIsVideoOn(!isVideoOn)}
                          isMain={gridParticipants.length === 1}
                          width={gridParticipants.length === 1 ? undefined : 100}
                          isTablet={isTablet}
                          height={gridParticipants.length === 1 ? undefined : 100}
                        />
                    ...
                    {remainingParticipantsCount > 0 && (
                      <div
                        className={`${activeParticipant? 'w-1/5':''} rounded-2xl relative cursor-pointer bg-green-100 flex  items-center justify-center text-green-800 font-medium`}
                        onClick={() => {
              ...
              <Tooltip>
                <TooltipTrigger>
                  <Button
                    variant="outline"
                    size="icon"
                    className={`rounded-3xl hover:bg- cursor-pointer hover:scale-105 transition-all duration-300 ease-in-out size-12 sm:size-14 ${isMicOn ? "bg-medium-green " : "bg-btn-primary"}`}
                    onClick={() => {
                      setIsMicOn(!isMicOn)
                      if (activeParticipant) {
              ...
              <Tooltip>
                <TooltipTrigger>
                  <Button
                    variant="outline"
                    size="icon"
                    className={`rounded-3xl hover:bg- cursor-pointer hover:scale-105 transition-all duration-300 ease-in-out size-12 sm:size-14 ${isVideoOn ? "bg-medium-green " : "bg-btn-primary"}`}
                    onClick={() => {
              ...
              <Tooltip>
                <TooltipTrigger>
                  <Button
                    variant="outline"
                    size="icon"
                    className={`rounded-3xl hover:bg- cursor-pointer hover:scale-105 transition-all duration-300 ease-in-out size-12 sm:size-14 ${isShareScreen ? "bg-medium-green " : "bg-btn-primary"}`}
